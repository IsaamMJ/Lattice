...
body += `<!-- WARNING: anything inside these markers is regenerated by \`lattice sync\`. Hand-edits to bullets are lost on next sync. To add notes, edit the YAML under .lattice/findings/open/<slug>.yml — that survives regen. -->\n\n`;

// --- Open (actionable) ---
// v0.7.7: OK is "audited and confirmed safe" — render it under Acknowledged,
// not Open. The `(N actionable)` count is the work-to-do count, OK excluded.
const openItems = byStatus.open.filter(f => (f.tier || '').toUpperCase() !== 'OK');
const okItems = byStatus.open.filter(f => (f.tier || '').toUpperCase() === 'OK');
...
// --- Acknowledged (OK — audited and confirmed safe) ---
if (okItems.length > 0) {
  body += `## Acknowledged (${okItems.length})\n\n`;
  body += `_Audited and confirmed safe — no action required. Surfaced here so the audit history isn't invisible._\n\n`;
  for (const f of okItems.sort((a, b) => (a.module || '').localeCompare(b.module || ''))) {
    body += renderSimpleLine(f) + '\n';
  }
  body += '\n';
}
