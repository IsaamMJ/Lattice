...
- **v0.9.7** (current) — friction predicates + `lattice review`. Reads the v0.9.6 MAT log and surfaces candidate bugs/UX gaps via five high-precision predicates: `fullpath_workaround`, `unknown_subcommand`, `repeated_failure`, `failed_then_succeeded`, `slow_command`. Output modes: pretty (default), `--json` (jq-pipeable), `--quiet` (count only). Local-only — no telemetry POST. v0.9.8 will add `--file` to feed candidates into `lattice report` with idempotent dedup. 105 → 111 lifecycle tests.
