...
- **v1.2.0** (current) — friction-reporting default-on (#42 #54) + framework-aware audits (#51) + `lattice diff` (#43) + `lattice write-manifest` (#48). SessionStart hook now carries an explicit DEFAULT-ON friction-filing protocol — Claude files via `lattice report` as friction occurs, not at session end. audit-sweep skill detects Next.js App/Pages router, monorepos, Go, Flutter, and flat-repo layouts before falling back to `src/modules/`. Plus fixed `node -` argv parsing bug that made `normalize --apply` a silent no-op.
