...
- **v0.7.10** (current) — milestone axis. New: optional `milestone:` field on findings (free-form: `p0-launch`, `v1.0`, `post-launch`) and `lattice list --milestone <name>` filter. Separates launch priority from severity tier — a LOW finding can be P0 for launch. Tests: 39 → 40. **v0.8.0 reserved** for cross-dimension dedupe, `Closes-Lattice:` commit convention, and JSON Schema validation.
