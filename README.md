...
- **v1.0.3** (current) — `lattice setup` single-command bootstrap (#2) + `lattice ci-check-dead` release-time pruning check (#7). New projects: one command instead of doctor + config init + sync. Release: `ci-check-dead` fails if any subcommand has 0 invocations in N days, enforcing the dead-feature pruning practice automatically.
