...
- **v2.1.0** (current) — closes the autonomy loop. `measurement.headers` block with `${VAR}` interpolation for auth-gated endpoints (#60). `measurement.sources` list + `combine: sum|max|min|weighted-avg` for bundled-change hypotheses (#60). `lattice grow attach-measurement` retrofits measurement blocks into pre-v1.4.2 YAMLs. `lattice grow check --json` envelope. `lattice grow schedule install` writes a GitHub Actions workflow that runs weekly + pipes results through `lattice-grow-telegram.mjs` → Telegram bot. One command per project; walk away; get verdicts in chat (#61).
