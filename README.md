...
- **v1.1.0** (current) — CLAUDE.md auto-tune Phase 2 (#6). `lattice claude-md-tune --check` reports trigger state (3+ releases since last tune, or 5+ unknown_subcommand events). `--auto` applies when triggered AND `LATTICE_AUTO_TUNE=1` is set. Every apply logs a UTC-timestamped diff to `~/.claude/lattice/claude-md-tune-history/`. Opt-in by design — default-on flip deferred until 30 days clean dogfood evidence.
