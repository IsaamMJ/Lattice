...
- **v0.9.5** (current) — enhancement slice clearing the remaining v0.9.3-dogfood requests. `lattice bulk-close --reason --rationale --pending` (#23) — migration scenarios close in one invocation instead of a per-slug for-loop. `lattice invariants derive --print` now ALSO persists `HEAD.yml` (#25) — fixes three subcommands disagreeing about whether invariants exist. `lattice decide --because-file <path>` and `--because -` (stdin) (#27) — paragraph-level ADR rationale via file or here-doc, emitted as YAML block scalar. 94 → 99 lifecycle tests. v0.9.3 → v0.9.4 → v0.9.5 is the first complete demo of the report-channel → fix-with-regression-tests feedback loop.
