...
- **v0.7.9** (current) — cross-finding state. New: `lattice changelog --since <date>` (renders closed findings as release-note markdown, grouped by close_reason then tier); optional `blocked_by:` field on findings; `lattice list --unblocked`/`--blocked` partition. Solves "what shipped in May?" and "what's actually ready to work on vs. waiting on a vendor?" without blurring the file:line boundary. Tests: 35 → 39.
