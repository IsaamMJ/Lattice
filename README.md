...
- **v0.9.3** (current) — manual report channel. `lattice report <category> --title --body` ships author-supplied observations through the same Cloudflare Worker as auto-telemetry, labelled `manual-report` so it's distinguishable from crash-fingerprint issues. Closes the silent-correctness blind spot — exit-0 bugs (parser malformations, UX gaps, docs) now have a wire. Worker extended to handle `kind: manual_report` + new fields; existing telemetry flow unchanged.
