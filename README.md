...
- **v0.9.11** (current) — project CLAUDE.md integration. New `lattice project-init` / `project-sync` / `project-restore` commands write a sentinel-managed state block (findings by tier, top-3 by priority, active ADRs, dimensions, telemetry status) into `<project>/CLAUDE.md`. Block is **auto-refreshed** on every `lattice close` and `lattice decide` (opt-out: `LATTICE_NO_PROJECT_SYNC=1`). Separate sentinels (`LATTICE-PROJECT-BLOCK-START`) from v0.9.10 global, so the two never collide. Project-scoped backups at `.lattice/claude-md-backups/`. Strict locked boundaries: no MEMORY.md logic (v0.9.12 evidence-gated), no MCP (v1.0.0). 127 → 134 lifecycle tests.
