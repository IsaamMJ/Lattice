...
| **CLI** (`lattice list`, `lattice next`, `lattice show`, `lattice sync`, `lattice close`, `lattice verify`, `lattice grow`, …) | Any shell, no model | **Manage** the lifecycle of those findings |
...
- **v2.1.3** (current) — three real-use bugs + Part A origin tracking. Closes #75 (generated workflow's `if: secrets.X` was rejected by GH Actions — now always runs + short-circuits internally), #67 (audit-env-contract excludes node_modules + vendored deps), #71 (literal-copy shim drift now detected via `lattice version` output comparison, catches ALL shim forms). New: `lattice report` tags each GH issue with `project:<basename>` label + adds `**Project:**` line in body, so `gh issue list --label project:X` surfaces per-project bug pressure.
...
- **v2.1.0** — closes the autonomy loop. `measurement.headers` block with `${VAR}` interpolation for auth-gated endpoints (#60). `measurement.sources` list + `combine: sum|max|min|weighted-avg` for bundled-change hypotheses (#60). `lattice grow attach-measurement` retrofits measurement blocks into pre-v1.4.2 YAMLs. `lattice grow check --json` envelope. `lattice grow schedule install` writes a GitHub Actions workflow that runs weekly + pipes results through `lattice-grow-telegram.mjs` → Telegram bot. One command per project; walk away; get verdicts in chat (#61).
- **v2.0.0** — closed-loop hypothesis execution. `lattice grow measure <slug>` fetches the live metric (http/file/cmd source schemes), compares against `success_threshold` + `window_days`, emits verdict. `lattice grow check` measures every running hypothesis. `lattice grow auto-rollback <slug> --execute` performs real `git revert` with backup ref (`refs/lattice/pre-rollback/<slug>`), transitions YAML, prints exact undo command. Verified end-to-end with a real git repo + file source. Cadence scheduling, confidence intervals, auto-PR, hypothesis auto-generation are explicit v2.1+ deferrals.
- **v1.4.2** — #58 fix (`body: unbound variable` in `mcp setup` / `wire-hooks` under set -euo pipefail; now defensively initialized) + #59 structured measurement fields in `lattice grow propose` (`--metric-name`, `--measurement-source`, `--baseline-value`, `--expected-delta`, `--success-threshold`, `--window-days`) and `--observed-value` on `grow close`. Lays the schema foundation v2.0 auto-rollback needs.
- **v1.4.1** — shim-drift detection in `lattice doctor` + self-verification in `lattice update --self`. Closes the silent-failure mode where the user's `lattice` shim resolves to a different on-disk script than update.sh writes, leading to "v1.4.0 shipped" + "machine still on 0.9.16" simultaneously being true. Both are now loud.
- **v1.4.0** — operator profile (#55 #56) + `lattice grow` hypothesis lifecycle (#57). New `lattice profile init|show|set` declares per-user workflow constraints (`scale: solo`, `will_check_dashboards: false`, etc.) that reweight finding tiers under `--effective-tier`. New `lattice grow propose|run|status|rollback|close` runs hypothesis lifecycle (open → running → closed/rolled-back) parallel to findings — foundation only; closed-loop auto-execution is v2.0.
- **v1.3.1** — `lattice list` hides `tier: OK` markers by default (#16). `--include-ok` / `--all` / `--tier OK` to see them. Matches what SessionStart hook + statusline already do for OK suppression.
- **v1.3.0** — new `lattice audit-infra` dimension (#36) detects missing Claude Code hooks + MCPs based on stack (typecheck hook on TS, env-protection on .env*, playwright/context7 on Next.js, etc). New `scripts/lattice-stop.mjs` Stop hook (#40) fires `lattice review --file --yes` at session end as a friction catch-net. `lattice wire-hooks` now wires Stop alongside SessionStart + statusLine.
- **v1.2.0** — friction-reporting default-on (#42 #54) + framework-aware audits (#51) + `lattice diff` (#43) + `lattice write-manifest` (#48). SessionStart hook now carries an explicit DEFAULT-ON friction-filing protocol — Claude files via `lattice report` as friction occurs, not at session end. audit-sweep skill detects Next.js App/Pages router, monorepos, Go, Flutter, and flat-repo layouts before falling back to `src/modules/`. Plus fixed `node -` argv parsing bug that made `normalize --apply` a silent no-op.
- **v1.1.2** — 6 fixes from the first real-world `/audit-sweep` dogfood. `lattice sync` now accepts nested-evidence YAMLs (#47 HIGH). `lattice list` no longer emits the `· :` tail for findings without file/line (#50). `lattice setup` drops `.lattice/.gitattributes` pinning YAML to LF (#53). audit-sweep skill auto-creates `.lattice/findings/{open,closed,sweeps}` (#49). New `lattice normalize [--apply]` (#44 #52) re-derives fabricated ids + strips leading-dot module segments + renames to canonical `TIER-MODULE-RULE.yml`; audit-sweep skill auto-runs it post-dispatch. Closes #41 (MCP) without code — already shipped in v1.0.0.
- **v1.1.1** — `lattice release-notes` (#11) auto-generates CHANGELOG.md entries from git commits since the last tag, categorized by conventional-commit prefix (feat/fix/refactor/perf/docs/test). Drops `release:` and merge commits. Embeds closed Lattice findings if `.lattice/findings/closed/` is populated. Kills the last hand-written release step.
- **v1.1.0** — CLAUDE.md auto-tune Phase 2 (#6). `lattice claude-md-tune --check` reports trigger state (3+ releases since last tune, or 5+ unknown_subcommand events). `--auto` applies when triggered AND `LATTICE_AUTO_TUNE=1` is set. Every apply logs a UTC-timestamped diff to `~/.claude/lattice/claude-md-tune-history/`. Opt-in by design — default-on flip deferred until 30 days clean dogfood evidence.
- **v1.0.4** — `update.sh` now refreshes the MCP server source (`mcp/package.json`, `tsconfig.json`, `src/index.ts`) + rebuilds `dist/` + regenerates the Windows `.cmd` shim. `lattice update --self` confirmed reliable on Windows + Git Bash via live v0.9.17 → v1.0.3 upgrade test.
- **v1.0.3** — `lattice setup` single-command bootstrap (#2) + `lattice ci-check-dead` release-time pruning check (#7). New projects: one command instead of doctor + config init + sync. Release: `ci-check-dead` fails if any subcommand has 0 invocations in N days, enforcing the dead-feature pruning practice automatically.
- **v1.0.2** — install-UX hardening. New `lattice uninstall` (#9, project + `--global` + `--purge`). `install.sh` now offers interactive auto-wire of hooks + MCP at the end (#5 — replaces the paste-snippet dance). `lattice doctor` detects first-run state and downgrades known-normal warnings to `[INFO]` so fresh installs no longer look like failures (#4). Statusline brand block carries signal: `[Lattice ✓]` / `[Lattice 12]` / `[Lattice ⨯]` (#8). SessionStart hook stops surfacing OK-tier markers as "findings to address" (#10).
- **v1.0.1** — PowerShell PATH fix (#46). `install.sh` now drops a `lattice.cmd` wrapper next to the bash shim on Windows and adds the shim dir to the Windows User PATH via PowerShell (idempotent, no admin). PowerShell + cmd.exe now resolve `lattice` natively — Git Bash detour eliminated. `lattice doctor` checks both bash `$PATH` and Windows registry `$env:PATH` separately. Why it wasn't in #37: Git Bash `$PATH` and Windows `$env:PATH` live in completely separate namespaces; treating them as one was the bug.
