...
- **v0.9.0** (current) — first slice of v1.0 substrate. New: `lattice mode`, `lattice decide`, `lattice decisions list/show`, ADR YAML schema at `.lattice/decisions/`, `LATTICE_OWNER_MODE=1` env. Full spec at `docs/v1.0-substrate-spec.md`. Invariant derivation + MAT traces + MCP server land in v0.9.x patches.
