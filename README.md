...
- **v0.8.3** (current) — `install.sh` auto-installs a `~/.local/bin/lattice` shim (symlink or wrapper) and patches PATH into the user's shell rc when missing (#13). Closes the loop with v0.8.2's PATH-detection diagnostic — `curl … | bash` → new shell → `lattice doctor` all-green, no manual alias.
