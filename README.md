...
- **v0.9.14** (current) — statusline rebuilt to match OMC HUD visual style. Two-line layout: info banner (`bold model | dim cwd | ⎇ branch`) above the main HUD (`[Lattice] | 5h:[bar]% | wk:[bar]% | Ctx:[bar]% | CRIT:N HIGH:M | N friction | mode`). OMC's ` | ` separator with dim styling. Severity-colored progress bars (green/yellow/red at 70/90% thresholds). `█` filled, `░` empty. `LATTICE_STATUSLINE_NOCOLOR=1` opt-out preserved. Lattice's own state surface (findings/friction/mode) — orchestration-specific OMC elements (ralph/autopilot/prd/etc) don't have analogs to port.
