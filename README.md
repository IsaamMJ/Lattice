...
- **v1.0.4** (current) — `update.sh` now refreshes the MCP server source (`mcp/package.json`, `tsconfig.json`, `src/index.ts`) + rebuilds `dist/` + regenerates the Windows `.cmd` shim. `lattice update --self` confirmed reliable on Windows + Git Bash via live v0.9.17 → v1.0.3 upgrade test.
