...
- **v1.4.1** (current) — shim-drift detection in `lattice doctor` + self-verification in `lattice update --self`. Closes the silent-failure mode where the user's `lattice` shim resolves to a different on-disk script than update.sh writes, leading to "v1.4.0 shipped" + "machine still on 0.9.16" simultaneously being true. Both are now loud.
