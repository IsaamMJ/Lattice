...
- **v1.4.2** (current) — #58 fix (`body: unbound variable` in `mcp setup` / `wire-hooks` under set -euo pipefail; now defensively initialized) + #59 structured measurement fields in `lattice grow propose` (`--metric-name`, `--measurement-source`, `--baseline-value`, `--expected-delta`, `--success-threshold`, `--window-days`) and `--observed-value` on `grow close`. Lays the schema foundation v2.0 auto-rollback needs.
