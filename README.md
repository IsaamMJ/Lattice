...
- **v0.9.1** (current) — second slice of v1.0 substrate. `lattice invariants derive` extracts stack + modules + edge functions + routes + DB tables from code; `lattice context` emits the session-start payload (mode + ADRs + invariants + findings summary). Verified on riseCraft frontend (flutter+supabase, 30 modules, 5 Edge Functions detected automatically).
